# AiBlocks Quickstart Docker Image

This project provides a simple way to incorporate aiblocks-core and millennium into your private infrastructure, provided that you use docker.
...
- Postgresql 9.5 is used for storing both aiblocks-core and millennium data
...
```shell
$ docker run --rm -it -p "8000:8000" -v "/home/scott/aiblocks:/opt/aiblocks" --name aiblocks aiblocks/quickstart --testnet
```

The `-v` option in the example above tells docker to mount the host directory `/home/scott/aiblocks` into the container at the `/opt/aiblocks` path.  You may customize the host directory to any location you like, simply make sure to use the same value every time you launch the container.  Also note: an absolute directory path is required.  The second portion of the volume mount (`/opt/aiblocks`) should never be changed.  This special directory is checked by the container to see if it is mounted from the host system which is used to see if we should launch in ephemeral or persistent mode.
...
To customize the configurations that both aiblocks-core and millennium use, you must use persistent mode.  The default configurations will be copied into the data directory upon launching a persistent mode container for the first time.  Use the diagram below to learn about the various configuration files that can be customized.

```
  /opt/aiblocks
  |-- core                  
  |   `-- etc
  |       `-- aiblocks-core.cfg  # AiBlocks core config
  |-- millennium
  |   `-- etc
  |       `-- millennium.env       # A shell script that exports millennium's config
  |-- postgresql
  |   `-- etc
...
| 8000  | millennium      | main http port       |
| 6060  | millennium      | admin port           |
| 11625 | aiblocks-core | peer node port       |
| 11626 | aiblocks-core | main http port       |


### Security Considerations

Exposing the network ports used by your running container comes with potential risks.  While many attacks are preventable due to the nature of the aiblocks network, it is extremely important that you maintain protected access to the postgresql server that runs within a quickstart container.  An attacker who gains write access to this DB will be able to corrupt your view of the aiblocks network, potentially inserting fake transactions, accounts, etc.

It is safe to open the millennium http port.  Millennium is designed to listen on an internet-facing interface and provides no privileged operations on the port. At the same time admin port should only be exposed to a trusted network, as it provides no security itself.

The HTTP port for aiblocks-core should only be exposed to a trusted network, as it provides no security itself.  An attacker that can make requests to the port will be able to perform administrative commands such as forcing a catchup or changing the logging level and more, many of which could be used to disrupt operations or deny service.

The peer port for aiblocks-core however can be exposed, and ideally would be routable from the internet.  This would allow external peers to initiate connections to your node, improving connectivity of the overlay network.  However, this is not required as your container will also establish outgoing connections to peers.
...
The command above assumes that you launched your container with the name `aiblocks`; Replace that name with whatever you chose if different.  When run, it will open an interactive shell running as root within the container.
...
```shell
millennium                          RUNNING    pid 143, uptime 0:01:12
postgresql                       RUNNING    pid 126, uptime 0:01:13
aiblocks-core                     RUNNING    pid 125, uptime 0:01:13
supervisor>
```
...
```shell
# restart millennium
supervisor> restart millennium  


# stop aiblocks-core
supervisor> stop aiblocks-core  
```
...
The point of this project is to make running aiblocks's software within your own infrastructure easier, so that your software can more easily integrate with the aiblocks network.  In many cases, you can integrate with millennium's REST API, but often times you'll want direct access to the database either millennium or aiblocks-core provide.  This allows you to craft your own custom sql queries against the aiblocks network data.

This image manages two postgres databases:  `core` for aiblocks-core's data and `millennium` for millennium's data.  The username to use when connecting with your postgresql client or library is `aiblocks`. The password to use is dependent upon the mode your container is running in:  Persistent mode uses a password supplied by you and ephemeral mode generates a password and prints it to the console upon container startup.
...
```
$ docker run -d -p "8000:8000" --name aiblocks aiblocks/quickstart --pubnet
```
